...
Learning Forward Solutions for LWR Model 
...
if load_model:
    model = torch.load('../models/{}_kmodel-{}.pt'.format(mainexpt,expt), 
...
K_comp_arr = np.zeros((len(eval_f_names), 2, eval_ntest_sc, s1, s2))
...
for f_name in eval_f_names:
    ...
    # Predicting over all test samples
    K_act,K_pred,K_rmse,K_mae = eval_test(eval_test_loader,eval_ntest,eval_s1,eval_s2)
...
np.save('../models/{}_kmae_results-{}.npy'.format(mainexpt, expt), K_mae_arr)
np.save('../models/{}_kcomp_results-{}.npy'.format(mainexpt, expt), K_comp_arr)
